...
const MeetingScreenPage = () => {
  ...
  return (
    <div className="flex flex-col h-[100vh] w-[100vw] bg-black">
      <div className="flex">
        <div className={`h-[calc(100vh-60px)] flex flex-wrap justify-center items-center gap-8 overflow-auto p-8 ${isChatBoxOpen && 'hidden'} md:inline-flex`}>
          <Participant />
          <Participant />
          <Participant />
          <Participant />
        ...
        <div className={`flex flex-col bg-black w-[70rem] h-[calc(100vh-60px)] px-8 md:px-4 pt-4 ${!isChatBoxOpen && 'hidden'}`}>
          <MessagesBox />
          <ComposeMessageBox />
        </div>
      </div>
